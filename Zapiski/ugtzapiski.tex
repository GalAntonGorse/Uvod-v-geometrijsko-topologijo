...
\begin{zgled}
  ...
  \begin{itemize}
    ...
    \item Če je $f: S^{n + 1} \to S^n$ homeomorfizem, potem je $B^n \cup_f B^n \cong S^n$.
...
\begin{trditev}
  ...
  obstaja homeomorfizem $h: \F P^n \to \F$, tako da je $h(u) = v$.
\end{trditev}
...
\begin{zgled}
  ...
  \begin{itemize}
    \item $\F P^0 = \quot{\F^1 \setminus \{0\}}{\F^*} = {\F^*}{\F^*} = \{a\}$.
...
\begin{zgled}
  ...
  Vsekakor pa lahko prebodan disk retraktiramo na $S^1$, in sicer z retrakcijo $r = \frac{x}{\|x\|}$.
\end{zgled}
...
\begin{izrek}[Jordan-Brouwer]
  Naj bo $r \geq 2$ in $S \subseteq \R^n$ topološka $(n-1)$-sfera, 
...
Prvo točko dokazujemo ob privzetku, da ima komplement vsaj dve komponenti (da je temu res tako, se bomo prepričalu v dokazu druge točke).
...
\begin{definicija}
  ...
  $S$ je lokalno ploščata ali podmnogoterost, če je lokalno ploščata v vsaki svoji točki.
    \item Naj bo $S^{n - 1} \cong S \subseteq \R^n$ in $x \in S$. Sfera $S$ je lokalno ploščata ali krotka v točki $x$,
  če obstaja okolica $U \subseteq S^n$ za $x$ in homeomorfizem $h : U \to W^{\ \text{odp}} \subseteq \R^n$,
  da je $h(S \cap U) = W \cap (\R^{n - 1} \times \{0\})$.
  $S$ je lokalno ploščata ali podmnogoterost, če je lokalno ploščata v vsaki svoji točki.
  \end{itemize}
\end{definicija}
...
\begin{zgled}
  ...
  \begin{itemize}
    ...
    \item Prostor zapolnjujoče krivulje (Hilbertova, Peanova) iz $I^2$ v $I$.
...
\begin{dokaz}
  ...
  vsebovano v $f(W)$. Naj bo $x \in f^{-1}(\{y\})$. Za $x \in W^{\ \text{odp}} \subseteq \R^n$
  ...
  Od tod pa naprej 
  $$y = f(x) \in f(\inte K)^{\ \text{odp v $\R^n$}} \subseteq f(U)$$
...
\begin{dokaz}
  ...
  vsebovana v $B$, torej $h(\inte A) \subseteq \inte B$.
...
Ta razdelek začnimo z intuicijo, da je mnogoterost dinmenzije $n$
...
\begin{dokaz}
  ...
  To pa pomeni, da je $y$ notranja točka v $N$, $f(U')^{\ \text{odp}} \subseteq N$ in je znotraj $f(W)$.
\end{dokaz}
...
\begin{dokaz}
  ...
  $\R^{n - 1}$. Naj bo $W$ evklidska okolica za $x$ v $M$,
  $h: W \to \R_+ ^n$ in $h(x) \in \R^{n - 1} \times \{0\}$.
...
\begin{dokaz}
  ...
  \begin{itemize}
    ...
    in $$(x, y) \in U_x \times V_y \cong \R^m \times \R^n_+ = \R^{m + n} \times [0, \infty) = R^{m + n}_+.$$
...
ima lahko več komponent. Če pa je povezana, potem pa lahko v $K = \bigsqcup U_i$
najdemo stranico, vzdolž katerih paroma zlepimo komponente tako, da 
...
\begin{definicija}
  ...
  $M$ je orientabilna, če lahko orientacije mnoogokotnikov $K_j$
...
\begin{itemize}
  ...
  \item \textbf{1-celice} v $M$ so slike odprtih stranic mnokotnikov (torej povezave grafa $\Gamma$).
...
\begin{definicija}
  ...
  $$\chi (M) = \# (\mathrm{2-celic}) - \# (\mathrm{1-celic}) + \#(\mathrm{0-celic}).$$
\end{definicija}
